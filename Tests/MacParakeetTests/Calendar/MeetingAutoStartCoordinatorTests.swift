...
final class MeetingAutoStartCoordinatorTests: XCTestCase {
    ...
    func testCalendarChangeClearsProbableManualStartSnapshotUntilRefreshCompletes() async throws {
        // Same gate as `testSettingsChangeNotificationTriggersImmediateRePoll`:
        // with `calendarEnabled` off, `coordinator.start()` returns before it
        // registers the calendar-change observer or polls, so there is nothing
