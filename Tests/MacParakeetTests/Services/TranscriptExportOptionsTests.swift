...
final class TranscriptExportOptionsTests: XCTestCase {
    ...
    func testResolvedKeepsAvailableOptionsOn() {
        // Speaker labels are requested explicitly: this fork's
        // `TranscriptExportOptions()` deliberately defaults them OFF for GUI
        // exports (see `SubtitleExportConfigTests`), and what this test is
        // about is that `resolved` never turns an AVAILABLE option off.
        let resolved = TranscriptExportOptions(includeSpeakerLabels: true).resolved(
