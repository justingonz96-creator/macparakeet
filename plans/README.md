...
|------|-------|--------|----------|-------------|
| [2026-09-03-per-prompt-inference-settings](active/2026-09-03-per-prompt-inference-settings.md) | Per-prompt LLM inference settings | **PROPOSED / OWNER GATE** | P2 | Draft spec and code-grounded plan for typed sampling/output/thinking settings on result prompts. Decide whether blank values inherit MacParakeet's historical defaults or mean raw provider defaults; then execute the six slices without changing unset-prompt behavior or widening settings to other LLM features. |
