...
> retrieval, and release framing. The **2026-09-07** documentation audit also
> reconciles the merged notes, inference-settings and speaker-correction slices;
> remaining older rows still need drift checks
...
|------|-------|--------|----------|-------------|
| [2026-09-05-post-meeting-notes-and-prompt-context](active/2026-09-05-post-meeting-notes-and-prompt-context.md) | Saved-meeting notes and opt-in prompt context | **IMPLEMENTED ON MAIN** | P2 | Merged via #959 (`c14b1ed4`). Editable notes, autosave, retained drafts, context snapshots and lifecycle gates. Historical design/QA remain in the plan; stable release availability is separate. |
| [2026-09-03-per-prompt-inference-settings](active/2026-09-03-per-prompt-inference-settings.md) | Per-prompt LLM inference settings | **IMPLEMENTED ON MAIN** | P2 | Integrated through #968 (`57e3391a`), superseding the #956 candidate state. Typed settings, boundary validation, provider filtering, queue snapshots, GUI configuration, CLI execution and effective-only receipts are implemented; historical plan verification is not current release qualification. |
| [2026-09-05-speaker-attribution-editing](active/2026-09-05-speaker-attribution-editing.md) | Speaker attribution editing and speaker management (#542, #900, #893 slice) | **PARTIAL — MERGED CORE** | P1 | #960 merged (`c79d3f52`): persistent corrections, effective-attribution reads, search/card invalidation, Timed-view editing with Undo/Redo, correction-aware exports, CLI, AI and artifacts. Broader artifact-retry UX, dedicated remove-split UI and replay work remain follow-ups; do not restart the implemented core. |
