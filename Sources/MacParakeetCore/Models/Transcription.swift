...
public struct Transcription: Codable, Identifiable, Sendable {
    ...
    ///
    /// Fork note: this was called `transcriptSegments` until the 2026-09
    /// upstream sync; upstream now owns that name for the speaker-attribution
    /// `[TranscriptSegmentRecord]` below (different shape, different column).
    /// Migration `v0.39-echo-engine-segments` moves the stored payload.
    public var engineSegments: [STTSegment]?
    ...
    public init(
        ...
        self.engineSegments = engineSegments
...
extension Transcription: FetchableRecord, PersistableRecord {
    ...
    public enum Columns: String, ColumnExpression {
        ...
        case rawTranscript, cleanTranscript, wordTimestamps, engineSegments, language
    ...
    public init(from decoder: Decoder) throws {
        ...
        // `engineSegments` was added in v0.10 (as `transcriptSegments`, renamed
        // by migration v0.39); old DB rows won't have it.
        engineSegments = try container.decodeIfPresent([STTSegment].self, forKey: .engineSegments)
