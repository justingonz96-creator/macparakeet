...
struct TranscriptResultView: View {
    ...
    private var presentationContent: some View {
        contextObservationContent
            ...
            .onChange(of: showingExportOptions) { _, newValue in
                if newValue {
        ...
        VStack(alignment: .leading, spacing: DesignSystem.Spacing.md) {
            HStack(alignment: .top, spacing: DesignSystem.Spacing.sm) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text(
                        "Echo can generate summaries, action items, and custom prompt results from this transcript. Transcription still works without AI."
                    )
        ...
        HStack(alignment: .top, spacing: DesignSystem.Spacing.sm) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text("Turn on AI for summaries and chat")
                    ...
                    .foregroundStyle(DesignSystem.Colors.textPrimary)
                Text(
                    "Echo can use a local AI app, your API key, or a command-line AI tool. Transcription still works without this."
                )
                .font(DesignSystem.Typography.bodySmall)
                .foregroundStyle(DesignSystem.Colors.textSecondary)
            }
    ...
    private var resolvedTranscriptExportOptions: TranscriptExportOptions {
        var options = transcriptExportOptions.resolved(
    ...
    private func exportToDownloads(format: TranscriptExportFormat) {
        ...
        if options.subtitleConfig.useLLMRefinement && (format == .srt || format == .vtt) {
            ...
            exportTask = Task { @MainActor in
                ...
                do {
                    // Flush any pending speaker edits before exporting, the
                    // same way the plain export path does.
                    let prepared = await viewModel.currentTranscriptionForSpeakerOutput()
                    guard activeTranscription.id == selectedID else { return }
                    guard let source = prepared, source.id == selectedID else {
                        exportErrorMessage =
                            "Couldn't prepare the current speaker changes. Please try exporting again."
                        return
                    }
